# -*- coding: utf-8 -*-
# ===================================
# ScriptName : node_ssh.py
# Author     : WFJ
# Email      : wfj_sc@163.com
# CreateTime : 2016-12-05 18:49
# ===================================
...
class NodeSSH(object):
    ...
    def init_ssh(self):
        ...
        try:
            # 首先是连接指定节点，成功后返回实例，错误则抛出异常
            self.ssh.connect(self.ip, self.port, self.user, self.passwd, timeout=30)
            return None
        except Exception, e:
            print "Error: ", e
            return "%s"%e

    def read_cmd_file(self, filename):
        cmd_string = ""
        with open(filename) as f:
            lines = f.readlines()
            cmd_string = ";".join([line for line in lines if not line.startswith("#")])
        return cmd_string

    def execute_script(self, filename):
        cmd = self.read_cmd_file(filename)
        return self.input_cmd([cmd])
    ...
    # 输入命令，对指定的实例输入命令，然后返回命令结果，错误则抛出异常
    def input_cmd(self, cmds):
        print "execute cmd: %s"%cmds
        result = ""
        note_cmd = "--- NOTE: execute cmd '%s' ---\n"
        for cmd in cmds:
            result += note_cmd%cmd
            i, o, e = self.ssh.exec_command(cmd)
            i.write("Y")
            result += "".join(o.readlines())
        return result


if __name__ == '__main__':
    h = NodeSSH('192.168.217.138', 'root', 'abc')
    print h.input_cmd(['A=`pwd`;echo $A;if [[ -d $A ]];then echo "yes";fi', 'pwd;echo "end cmd"'])
    print "test-----------------"
    cmd = h.read_cmd_file("test_cmd.sh")
    print h.input_cmd([cmd])
    h.close_conn()
